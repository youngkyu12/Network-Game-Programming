...
#include "GraphicsPipeline.h" // CGraphicsPipeline ���� ����(E0276) �ذ�
...
void CGameFramework::ProcessInput()
{
	...
	static bool A_PressedPrev = false; // �������� ���
	static bool F_PressedPrev = false; // �������� ���
	static bool P_PressedPrev = false; // �������� ���
	
	static UCHAR pKeyBuffer[256];
	if ( GetKeyboardState ( pKeyBuffer ) )
	{
		keyPKT.keyW = (pKeyBuffer['W'] & 0xF0) ? 1 : 0;// char w ����
		keyPKT.keyS = (pKeyBuffer['S'] & 0xF0) ? 1 : 0; // char s ����
...
void CGameFramework::RenderScene()
{
	...
	drawList.reserve(static_cast<size_t>((m_pScene ? m_pScene->m_nObjects : 0)) + static_cast<size_t>(MAX_PLAYERS)); // lnt-arithmetic-overflow ��� �ذ�
	...
	{
		// �ʿ� �� ������ alpha�� ä���: RenderFilled(hdc, pCamera, color, alpha)
...
void CGameFramework::HandlePacket()
{
	...
	while (recv_Queue.pop(player)) 
	{
		...
		{	//GameObject���� Player�� ��ȯ ���� m_pPlayer -> m_pPlayer[0]
			...
			m_pPlayer[0]->PrevFire = player.fire; // ���� ���¸� ������ ���°����� ����.(���� �� ����� ���ؼ�)
		...
		else
		{
			...
			if ( m_pScene && objIndex >= 0 && objIndex < MAX_PLAYERS ) //10������
			{
				//GameObject���� Player�� ��ȯ ���� m_pPlayer[objIndex]
...
void CGameFramework::DrawUI()
{
	...
	const int bannerMarginTop = 8;    // ��� ����
	const int bannerHeightPx = 36;    // ��Ʈ ���� (�ȼ�). ũ��/�۰� ����
	const int bannerWeight = FW_BOLD; // ����(FW_NORMAL, FW_BOLD ��)
	...
	int bannerX = m_rcClient.left + (clientW - bannerSize.cx) / 2; // �߾� ���� X
	int bannerY = bannerMarginTop;                                 // ��� ���� Y
...
void CGameFramework::TriggerGameOver ( const char* reason ) {
	...
	// ���� ��ŷ ����: ������(���) �� ���� ����(�տ������� ����)
	m_vFinalRanks.clear ();
	for ( auto id : m_alivePlayers ) m_vFinalRanks.push_back ( id ); // ���� �� ���� �� ������ ���
...
void CGameFramework::RenderResultScene ( HDC hdc ) {
	...
	for ( size_t i = 0; i < m_vFinalRanks.size (); ++i ) {
		TCHAR line[64];
		_stprintf_s ( line , _T ( "%zu��: Player %u" ) , i + 1 , ( unsigned )m_vFinalRanks[i] );
	...
	// �ȳ�
	y += dy;
	TextOut ( m_hDCFrameBuffer , x , y , _T ( "ESC: ���� " ) , 8 );
}
